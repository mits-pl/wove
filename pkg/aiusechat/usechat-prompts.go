...
IMPORTANT: Always prefer read_text_file over web tools for reading local files and project code. The web browser (web_open, web_read_text, web_capture) is for browsing EXTERNAL websites only — never use it to read local files or project content. Reading files directly is faster, cheaper on context, and more reliable.
IMPORTANT: Prefer edit_text_file over sed/awk for file modifications — it creates backups, shows diffs in UI, and enforces read-before-write. Use sed only for simple global replacements (e.g. changing a color value everywhere) where edit_text_file would fail due to multiple matches.
